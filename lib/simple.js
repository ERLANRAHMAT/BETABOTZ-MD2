...
const { resolveLidViaMeta, fetchGroupMetadataRaw } = require('./baileys')
...
`.trim()
    ...
    */
        ...
        for (const message of m) {
            try {
                ...
                function isFromMe(conn, m) {
                    ...
                    return (
                        areJidsSameUser(sender, conn.decodeJid(conn.user.id)) ||
                        areJidsSameUser(sender, conn.decodeJid(conn.user.lid))
                    )
                }
                const fromMe = isFromMe(conn, m)
     ...
     * @param {Object} m
     */
    conn.serializeM = async (m) => {
        return await exports.smsg(conn, m)
    }
...
exports.smsg = async (conn, m, hasParent) => {
    ...
    if (m.key) {
        if (m.key.participant) {
            const rawKeyPart = conn.decodeJid(m.key.participant)
            m.key.participant = await resolveLidViaMeta(rawKeyPart, conn.decodeJid(m.key.remoteJid), conn)
        }
        m.id = m.key.id
        ...
        m.senderRaw = conn.decodeJid(
            m.key.fromMe
                ? (conn.user.lid || conn.user.id)
                : (m.participant || m.key.participant || m.chat || '')
        )

        m.sender = await resolveLidViaMeta(m.senderRaw, conn.decodeJid(m.key.remoteJid || message.message?.senderKeyDistributionMessage?.groupId || ''), conn)

        m.fromMe = areJidsSameUser(m.sender, conn.decodeJid(conn.user.id)) ||
            areJidsSameUser(m.sender, conn.decodeJid(conn.user.lid))
    ...
    if (m.message) {
        ...
        if (m.mtype == 'protocolMessage' && m.msg.key) {
            ...
            const lid = conn.decodeJid(conn.user.lid)
            const partRaw = m.msg?.key?.participant || m.msg?.key?.remoteJid || ''
            const part = conn.decodeJid(partRaw)
            const rjid = conn.decodeJid(m.msg.key.remoteJid)
            m.msg.key.fromMe = areJidsSameUser(part, jid) || areJidsSameUser(part, lid)

            if (!m.msg.key.fromMe && (areJidsSameUser(rjid, jid) || areJidsSameUser(rjid, lid))) {
        ...
        const rawQuotedMentioned = m.msg?.contextInfo?.mentionedJid?.length && m.msg.contextInfo.mentionedJid || []
        m.mentionedJid = await Promise.all(
            rawQuotedMentioned.map(jid => resolveLidViaMeta(jid, m.chat, conn))
        )
        ...
        if (m.quoted) {
            ...
            if (m.quoted) {
                ...
                const rawQuotedSender = conn.decodeJid(m.msg.contextInfo.participant)
                m.quoted.sender = await resolveLidViaMeta(rawQuotedSender, m.chat, conn)

                m.quoted.fromMe =
                    areJidsSameUser(m.quoted.sender, conn.decodeJid(conn.user.id)) ||
                    areJidsSameUser(m.quoted.sender, conn.decodeJid(conn.user.lid))

                m.quoted.text = m.quoted.text || m.quoted.caption || m.quoted.contentText || ''
                m.quoted.name = conn.getName(m.quoted.sender)
                const rawQuotedMentioned = m.quoted.contextInfo?.mentionedJid || []
                m.quoted.mentionedJid = await Promise.all(
                    rawQuotedMentioned.map(jid => resolveLidViaMeta(jid, m.chat, conn))
                )
