...
exports.attach = (conn) => {
 conn.isLid = new NodeCache({
    ...
    if (conn._client?.removeAllListeners) {
        conn._client.removeAllListeners('offline_resume');
    }
    conn._client?.on?.('offline_resume', (ev) => {
        conn.isOfflineResuming = ev.status === 'resuming';
        if (ev.status === 'resuming') {
            conn.logger?.info?.(`⏳ Offline resume: ${ev.totalStanzas} pending stanzas, skipping old messages`);
        } else {
            conn.logger?.info?.('✅ Offline resume complete, processing new messages');
        }
    });
    const seedOwnLid = () => {
        try {
    ...
    conn.sendMessage = async (jid, content, options = {}) => {
        // -antispam ketika awal startup dengan sessions lawas
        if (global.timestamp?.connect) {
            let uptime = Date.now() - (global.timestamp.connect * 1);
            if (uptime < 5 * 60 * 1000) { 
                conn.startupSpamCheck = conn.startupSpamCheck || { count: 0, lastTime: Date.now(), cooldownEnds: 0 };
                let now = Date.now();
                
                if (now < conn.startupSpamCheck.cooldownEnds) {
                    return {}; 
                }
                
                if (now - conn.startupSpamCheck.lastTime > 1000) {
                    conn.startupSpamCheck.count = 0;
                    conn.startupSpamCheck.lastTime = now;
                }
                
                conn.startupSpamCheck.count++;
                if (conn.startupSpamCheck.count > 5) { 
                    conn.startupSpamCheck.cooldownEnds = now + 5 * 60 * 1000; // ubah disini jika ingin cooldown nya tidak lama
                    console.log(chalk.red('[SPAM DETECTED] Terlalu banyak pesan saat startup! Mengaktifkan cooldown'));
                    return {};
                }
            }
        }
