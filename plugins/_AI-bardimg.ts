...
let handler: WaPlugin = async (m, { conn, text, command, usedPrefix }) => {
  ...
  try {
    if (/image/.test(mime) && !/webp/.test(mime)) {
      ...
      baseUrl = `https://api.betabotz.eu.org/api/search/bard-img?url=${media}&text=${text}&apikey=${lann}`;
    } 
    else if (/video/.test(mime)) {
      if (q.seconds > 60) throw 'Maximum video duration is 60 seconds!';
      let buffer = await q.download();
      media = await uploadFile(buffer);
      baseUrl = `https://api.betabotz.eu.org/api/search/bard-video?url=${media}&text=${text}&apikey=${lann}`;
    }
    else if (/audio/.test(mime)) {
      let buffer = await q.download();
      media = await uploadFile(buffer);
      baseUrl = `https://api.betabotz.eu.org/api/search/bard-audio?url=${media}&text=${text}&apikey=${lann}`;
    }
    else {
