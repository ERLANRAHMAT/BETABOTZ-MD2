let handler = async (m, { conn, usedPrefix, text }) => {
  try {
    ...
    if (!who)
      return conn.reply(
        m.chat,
        `Tag atau reply orang yang ingin ditembak!\nContoh: ${usedPrefix}tembak @tag`,
        m,
      );
    ...
    if (
      sender.pasangan &&
      sender.pasangan !== who &&
      global.db.data.users[sender.pasangan]?.pasangan === m.sender
    ) {
      let denda = Math.ceil((sender.exp / 1000) * 20);
      sender.exp -= denda;
      return conn.reply(
        m.chat,
        `Kamu sudah punya pasangan @${sender.pasangan.split("@")[0]}!\nPutus dulu dengan ${usedPrefix}putus @tag\nDenda selingkuh: ${format(denda)} exp`,
        m,
    ...
    if (
      ...
      global.db.data.users[user.pasangan]?.pasangan === who
    ) {
      let denda = Math.ceil((sender.exp / 1000) * 10);
      sender.exp -= denda;
      return conn.reply(
        m.chat,
        `Dia sudah punya pasangan @${user.pasangan.split("@")[0]}!\nDenda ngege: ${format(denda)} exp`,
        m,
    ...
    if (user.pasangan === m.sender) {
      ...
      return conn.reply(
        m.chat,
        `Selamat! @${m.sender.split("@")[0]} & @${who.split("@")[0]} resmi pacaran!\nSemoga langgeng`,
        m,
    ...
    conn.reply(
      m.chat,
      `@${who.split("@")[0]} kamu ditembak @${m.sender.split("@")[0]}!\n\nKetik *${usedPrefix}terima @${m.sender.split("@")[0]}* untuk menerima\nKetik *${usedPrefix}tolak @${m.sender.split("@")[0]}* untuk menolak`,
      m,
