...
let handler: WaPlugin = async (m, { conn, usedPrefix }) => {
  try {
    ...
    if (id in conn.tebakpokemon) {
      await conn.reply(m.chat, 'Masih ada soal belum terjawab di chat ini', conn.tebakpokemon[id][0]);
      return;
    }

    let json;
    try {
      let src = await (await fetch(`https://api.betabotz.eu.org/api/game/tebakpokemon?apikey=${lann}`)).json();
      json = src;
    } catch (e) {
      console.log(e);
      throw e;
    ...
    if (!json || !json.img || !json.jawaban) throw new Error('Format data tebakpokemon tidak valid dari API.');
