...
async function handler(this: WaConnection, m, { conn, text }) {
    ...
    if (m.sender in conn.tambang) {
        ...
        if (text === 'start') {
            ...
            let { area, txt, reward } = tambangs[0];
            let currentArea = 0;
            let hasilTambang = 0;
...
handler.before = async m => {
    ...
    if (msg === 'stop') {
        ...
        if (areas[currentArea].txt === msg) {
            let { area, reward } = areas[currentArea] as { area?: string; txt?: string; reward?: { exp?: number; resources?: Record<string, number> } };
            user.exp += reward.exp;

            const resNum = (reward.resources as Record<string, number>);
            for (let resource in reward.resources) {
                (user.resources as Record<string, number>)[resource] = ((user.resources as Record<string, number>)[resource] || 0) + (resNum[resource] || 0);
                (totalReward as Record<string, number>)[resource] += (resNum[resource] || 0);

                // Ensure the updated resources are saved to the database
                (global.db.data.users[m.sender] as Record<string, number>)[resource] = ((global.db.data.users[m.sender] as Record<string, number>)[resource] || 0) + (resNum[resource] || 0);
            }
