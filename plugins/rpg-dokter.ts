...
const handler: WaPlugin = async (m, { conn, usedPrefix, command, args }) => {
  ...
  const player = conn.playerr[sender] || { Balance: 0, Pasien_Sembuh: 0, Waktu_Sembuh: 0, Obat_Super: 0, Lv: 1, State: STATES.IDLE } as WaGameRoom;
  ...
  if (command === "dokter") {
    ...
    conn.playerr[sender] = player as unknown as WaGameRoom;
