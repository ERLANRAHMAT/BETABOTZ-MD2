...
let nomors = m.sender
  ...
  let __timers = (Date.now() - global.db.data.users[m.sender].lastrob)
  let _timers = (3600000 - __timers) 
  let timers = clockString(_timers)
  let users = global.db.data.users
  if (Date.now() - global.db.data.users[m.sender].lastrob > 3600000){
  if (10000 > users[who].money) throw 'Target Gaada Uang bodoh, Kismin dia'
  users[who].money -= dapat * 1
  users[m.sender].money += dapat * 1
  global.db.data.users[m.sender].lastrob = Date.now()
