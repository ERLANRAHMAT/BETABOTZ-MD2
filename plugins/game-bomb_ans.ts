...
let json = (conn.bomb[id][1] as unknown as { emot?: string; number?: string; position?: number; state?: boolean; [key: string]: unknown }[]).find(v => v.position == +body);
if (!json) return conn.reply(m.chat, `🚩 Untuk membuka kotak kirim angka 1 - 9`, m);

if (json.emot == '💥') {
json.state = true;
let bomb = conn.bomb[id][1] as unknown as { emot?: string; number?: string; position?: number; state?: boolean; [key: string]: unknown }[];
let teks = `乂  *B O M B*\n\n`;
...
json.state = true;
let changes = conn.bomb[id][1] as unknown as { emot?: string; number?: string; position?: number; state?: boolean; [key: string]: unknown }[];
