...
let handler: WaPlugin = async (m, { conn, args, participants }) => {
  ...
  const boards: [string, string, string, Sorted[]][] = [
  ...
  const rankOf = (sorted: Sorted[], jid: string) => {
  ...
  const boardText = async (title: string, prop: string, label: string, sorted: Sorted[]) => {
...
function sort(property: string, ascending = true): (...args: Sorted[]) => number {
  if (property) return (...args: Sorted[]) => args[ascending ? 1 : 0][property] - args[ascending ? 0 : 1][property]
  else return (...args: Sorted[]) => +args[ascending ? 1 : 0][property] - +args[ascending ? 0 : 1][property]
}

function toNumber(property: string, _default = 0): SortCb {
  if (property) return (a: Sorted, i = 0, b = [] as Sorted[]) => {
