...
let handler: WaPlugin = async (m, { conn, command, args }) => {
    ...
    try {
        if (command === 'ytlive') {
            ...
            const timeSinceLastLive = Date.now() - lastLiveTime;
            ...
            if (user.subscribers >= 1000000 && user.playButton < 3) {
                user.playButton += 1;
                user.eris += Math.floor(Math.random() * (1000000 - 500000 + 1)) + 500000;
                user.exp += 5000;
                ...
                user.playButton += 1;
                user.eris += Math.floor(Math.random() * (500000 - 300000 + 1)) + 300000;
                user.exp += 2500;
                ...
                user.playButton += 1;
                user.money += Math.floor(Math.random() * (250000 - 10000 + 1)) + 10000;
                user.exp += 500;
...
// Convert large numbers to K, Jt, M, T format
...
// Convert a number to Indonesian currency (IDR) format
