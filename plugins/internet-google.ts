...
const handler: WaPlugin = async (m, { conn, command, text }) => {
  if (!text) return conn.reply(m.chat, 'Masukkan teks untuk dicari!', m);

  try {
    await m.reply(wait)
    const response = await fetch(`https://api.betabotz.eu.org/api/search/google?text1=${encodeURIComponent(text)}&apikey=${lann}`);
