...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    try {
        ...
        let apiUrl = `https://api.betabotz.eu.org/api/maker/canvas-fakeMl?apikey=${lann}&avatar=${encodeURIComponent(link)}&border=${borderNum}&rank=${encodeURIComponent(rank.trim().toLowerCase())}&username=${encodeURIComponent(username.trim())}`;
