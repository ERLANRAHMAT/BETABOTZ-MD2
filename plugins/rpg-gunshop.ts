...
const handler: WaPlugin = async (m, { conn, command, usedPrefix, args, text, isPrems }) => {
    ...
    if (Date.now() - +(user.pekerjaantiga || 0) < cooldown) {
        const remainingTime = new Date(+(user.pekerjaantiga || 0) + cooldown).getTime() - Date.now();
...
const listItems = Object.fromEntries(Object.entries(items[command.toLowerCase()])
    .filter(([v, { money, dana, gopay, ovo }]: [string, { money?: number; dana?: number; gopay?: number; ovo?: number }]) => {
...
if (command.toLowerCase() == 'buygun') {
    ...
    const reward = (items[command.toLowerCase()] as Record<string, Record<string, unknown>>)[item] ?? {};
    const rewardKey = Object.keys(reward)[0];
    if (!(rewardKey in (user as object))) {
