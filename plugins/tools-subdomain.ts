...
async function whois(subdomain, domain) {
  const url = `https://api.botcahx.eu.org/api/tools/whois-subdo?subdomain=${encodeURIComponent(subdomain)}&domain=${encodeURIComponent(domain)}&apikey=${lann}`;
  try {
...
async function create(subdomain, domain, type, content, proxied) {
  const url = `https://api.botcahx.eu.org/api/tools/create-subdo?subdomain=${encodeURIComponent(subdomain)}&domain=${encodeURIComponent(domain)}&type=${encodeURIComponent(type)}&content=${encodeURIComponent(content)}&proxied=${proxied}&apikey=${lann}`;
  try {
    const response = await fetch(url, { method: 'GET' });
    return await response.json();
  } catch (error) {
