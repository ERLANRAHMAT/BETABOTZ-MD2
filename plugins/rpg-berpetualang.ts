...
const cooldown = 5 * 60 * 1000; // 5 minutes cooldown
            ...
            let { area, txt, reward } = adventures[0];
            let currentArea = 0;
...
handler.before = async m => {
    ...
    if (msg === 'stop') {
        ...
        if (areas[currentArea].txt === msg) {
            ...
            user.exp += reward.exp;
            const lootNum = (reward.loot as Record<string, number>);
            for (let item in reward.loot) {
                (user.loot as Record<string, number>)[item] = ((user.loot as Record<string, number>)[item] || 0) + (lootNum[item] || 0);
                (totalReward as Record<string, number>)[item] += (lootNum[item] || 0);
            }
            hasilPetualangan++;
            currentArea++;
