    ...
    try {
        ...
        try {
            ...
            if (command === 'togif' || command === 'tomp4') {
                json = await (await fetch(`https://api.betabotz.eu.org/api/tools/webp2mp4?url=${media}&apikey=${btc}`)).json();
            } else if (command === 'toimg') {
                json = await (await fetch(`https://api.betabotz.eu.org/api/tools/webp2png?url=${media}&apikey=${btc}`)).json();
            }
            if (json && json.result) {
