...
let handler: WaPlugin = async (m, { conn, text, isPrems, isROwner, usedPrefix, command }) => {
    ...
    if (id in conn.skata) {
        ...
        if (room.status == 'play') {
            if (!(room.waktu as { _destroyed?: boolean })._destroyed && !room.diam) return conn.reply(m.chat, `Hii @${m.sender.split('@')[0]}, Masih ada game berlangsung di chat ini\nTunggu hingga game berakhir\nLalu ikut bergabung`, room.chat, { contextInfo: { mentionedJid: member } }).catch(e => { return !1 });
...
Dan hanya bisa dimainkan oleh player yang terdaftar`.trim();
        ...
        conn.skata[id] = {
            ...
            waktu: false as unknown as ReturnType<typeof setTimeout>
        };
    }
};
...
function filter(text: string) {
    ...
    // consonant {kuku, batu, kamu, aku, saya, etc.}
    else {
