...
conn 
  ...
  type CpuWithTotal = ReturnType<typeof os.cpus>[number] & { total: number }
  const cpus: CpuWithTotal[] = os.cpus().map(cpu => {
    ;(cpu as CpuWithTotal).total = Object.keys(cpu.times).reduce((last, type) => last + cpu.times[type], 0)
    return cpu as CpuWithTotal
  })
  ...
  var _muptime
    if (process.send) {
      process.send('uptime')
      _muptime = (await new Promise<number>(resolve => {
        process.once('message', resolve as (message: unknown, sendHandle: unknown) => void)
