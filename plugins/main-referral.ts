...
`.trim(), link_creator)
    ...
    let code = users[m.sender].ref_code = users[m.sender].ref_code || new Array(11).fill(undefined).map(() => [...'0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz'][crypto.randomInt(62)]).join('')
