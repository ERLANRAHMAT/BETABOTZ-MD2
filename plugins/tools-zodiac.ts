let handler: WaPlugin = (m, { usedPrefix, command, text }) => {
    ...
    if (isNaN(date.getTime())) throw date
    const d = new Date()
    ...
    const selisih = +new Date(1970, bulan - 1, tanggal) - +new Date(1970, birth[1] - 1, birth[2])
    const birthday = [tahun + (selisih > 0 ? 1 : 0), ...birth.slice(1)]
