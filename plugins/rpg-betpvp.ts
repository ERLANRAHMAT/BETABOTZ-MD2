...
let handler: WaPlugin = async (m, { conn, args, usedPrefix, command }) => {
    ...
    let count = (args[1] && number(parseInt(args[1])) ? Math.max(parseInt(args[1]), 1): /all/i.test(args[1]) ? Math.floor(parseInt(String(user[m.sender][type]))): 1) * 1
    let id = 'judipvp_' + Date.now()
