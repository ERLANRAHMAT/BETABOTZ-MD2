...
exports.before = async function (m) {
    ...
    if (room) {
        if (m.sender === room.p2 && /y(a|es)?/i.test(m.text.toLowerCase()) && m.isGroup && room.status === 'wait') {
            ...
            if ((user as unknown as Record<string, Record<string, number>>)[room.p][room.type] < room.taruhan) return m.reply(`Uang Lawanmu Kurang! membutuhkan ${room.taruhan} ${room.type}`);
            ...
            if ((user as unknown as Record<string, Record<string, number>>)[room.spin][room.type] < room.taruhan) return m.reply(`Uang Kamu Kurang! Kamu membutuhkan ${room.taruhan} ${room.type}`);
