...
let handler: WaPlugin = async (m, { conn, command, args, usedPrefix }) => {
    ...
    let __timers = (Date.now() - users.lastkerja)
    ...
    if (/^kerja$/i.test(command)) {
        switch (type) {
            case 'ojek':
                if (Date.now() - users.lastkerja < 300000) return m.reply(`Kamu sudah bekerja\nSaatnya istirahat selama ${clockString(time - Date.now())}`)
                ...
                break
            case 'pedagang':
                if (Date.now() - users.lastkerja < 300000) return m.reply(`Kamu sudah bekerja,Saatnya istirahat selama\n🕜 ${clockString(time - Date.now())}`)
