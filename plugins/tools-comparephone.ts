...
let handler: WaPlugin = async (m, { conn, text, usedPrefix, command }) => {
    ...
    try {
        await m.reply(wait);
        let json = await fetch(`https://api.betabotz.eu.org/api/tools/compare-phone?hp1=${encodeURIComponent(phone1)}&hp2=${encodeURIComponent(phone2)}&apikey=${lann}`).then(res => res.json());
        let res = json.result;
