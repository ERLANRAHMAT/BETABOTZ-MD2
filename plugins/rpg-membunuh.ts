...
let nomors = m.sender
  ...
  let __timers = (Date.now() - global.db.data.users[m.sender].lastbunuhi)
  let _timers = (3600000 - __timers) 
  let timers = clockString(_timers)
  let users = global.db.data.users
  if (Date.now() - global.db.data.users[m.sender].lastbunuhi > 3600000){
  ...
  global.db.data.users[m.sender].lastbunuhi = Date.now()
