let handler: WaPlugin = async (m, { conn, usedPrefix, owner }) => {
    try {
        ...
        let __timers = (Date.now() - user.lastadventure)
        let _timers = (600000 - __timers) // 10 minutes in milliseconds
        let timers = clockString(_timers)
        if (user.healt > 79) {
            if (Date.now() - user.lastadventure > 600000) { // 10 minutes cooldown
                // Define monsters
                let monsters = [
...
let str = `
🩸 Nyawa mu berkurang -${+healt} karena Kamu telah berpetualang sampai ${peta} dan melawan ${enemy.name}. Kamu mendapatkan:
✨ *Exp:* ${exp}
💰 *Uang:* ${uang}
🎫 *Tiketcoin:* 1
🗑️ *Sampah:* ${sampah}${+(potion || 0) == 0 ? '' : '\n🧪 *Potion:* ' + potion + ''}${+(diamond || 0) == 0 ? '' : '\n💎 *Diamond:* ' + diamond + ''}${+(common || 0) == 0 ? '' : '\n📦 *Common crate:* ' + common + ''}${+(uncommon || 0) == 0 ? '' : '\n🎁 *Uncommon crate:* ' + uncommon + ''}
`.trim()
