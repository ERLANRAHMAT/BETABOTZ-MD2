...
let handler: WaPlugin = async (m, { conn, text, usedPrefix, command }) => {
    ...
    let apiUrl = `https://api.betabotz.eu.org/api/maker/canvas-fakeMl?username=${encodeURIComponent(username.trim())}&border=${borderNum}&rank=${encodeURIComponent(rank.trim().toLowerCase())}&avatar=${encodeURIComponent(link)}&apikey=${lann}`;
