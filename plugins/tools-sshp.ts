...
var handler: WaPlugin = async (m, { conn, command, args }) => {
  ...
  try {
    var img = await fetch(`https://api.betabotz.eu.org/api/tools/sshp?url=${url}&apikey=${lann}`);
    if (!img) {
      await m.reply('Gagal saat percobaan pertama. Memulai percobaan kedua...');
      img = await fetch(`https://api.betabotz.eu.org/api/tools/sshp?url=${url}&apikey=${lann}`);
