let handler = async (m, { conn, usedPrefix }) => {
  try {
    ...
    if (!who)
      return conn.reply(m.chat, `Tag atau reply orang yang menembakmu!`, m);

    if (who === m.sender) return conn.reply(m.chat, `Tidak masuk akal`, m);
    if (who === conn.user.jid)
      return conn.reply(m.chat, `Bot tidak bisa pacaran`, m);
    ...
    if (user.pasangan !== m.sender) {
      return conn.reply(
        m.chat,
        `Maaf @${who.split("@")[0]} tidak sedang menembakmu`,
        m,
        { mentions: [who] },
      );
    }
    ...
    conn.reply(
      m.chat,
      `Selamat! @${m.sender.split("@")[0]} & @${who.split("@")[0]} sekarang resmi pacaran!\n\nSemoga langgeng dan bahagia selalu`,
      m,
