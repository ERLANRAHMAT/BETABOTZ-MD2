...
let handler: WaPlugin = async (m, { conn, usedPrefix: _p, args = [], command }) => {
    try {
        let pkg = JSON.parse(String(await fs.promises.readFile(path.join(__dirname, '../package.json')).catch(_ => '{}')))
