...
const handler: WaPlugin = async (m, { conn, usedPrefix, command, args }) => {
  const sender = m.sender;
  const user = global.db.data.users[sender]
  conn.players = conn.players || {};
  const player = conn.players[sender] || { Uang: 0, Pencuri_Tertangkap: 0, Waktu_Tertangkap: 0, Kaca_Pembesar: 0, Level: 1, State: STATES.IDLE, ThiefAction: undefined };
  ...
  if (command === "polisi") {
    ...
    conn.players[sender] = player as unknown as WaGameRoom;
