...
import simple, { generateMessageID } from './lib/simple.js';
import {
	WaClient,
	createStore,
	createNoopLogger,
...
async function fetchLatestWaWebVersion() {
	try {
		...
		return { version: [2, 3000, 1043028647], isLatest: false };
	}
}
...
export function makeWASocketBase(connectionOptions = {}) {
	...
	client.on('auth_qr', () => { client._zapoPairingReady = true; });
	client.on('auth_pairing_required', () => { client._zapoPairingReady = true; });
	...
	conn.ws = {
		readyState: 1, // CONNECTING=0, OPEN=1 (approximasi)
	...
	client.on('group', (ev) => {
		...
		if (!stub && !GROUP_ACTION_MAP[ev.action]) {
			//console.log('[group-event] unhandled action:', ev.action, '| gid:', gid);
		}
		if (stub && gid) {
	...
	client.on('presence', (ev) => {
		const id = ev.chatJid;
		const sender = ev.rawNode?.attrs?.from || ev.jid;
		if (!id || !sender) return;
		conn.ev.emit('presence.update', {
			id,
			presences: {
				[sender]: {
					lastKnownPresence: ev.presenceType || 'available',
	...
	conn.requestPairingCode = async (phoneNumber, customCode) => {
		...
		if (!state.registered && !client._zapoPairingReady) {
			...
			await new Promise((resolve) => {
				const timer = setTimeout(() => { clearInterval(iv); resolve(); }, 45000);
				const iv = setInterval(() => {
					if (client._zapoPairingReady) { clearInterval(iv); clearTimeout(timer); resolve(); }
				}, 500);
			});
		}
		for (let attempt = 1; ; attempt++) {
			try {
				return await client.auth.requestPairingCode(String(phoneNumber).replace(/\D/g, ''), false, sanitizePairingCode(customCode));
			} catch (e) {
				if (attempt >= 5) throw new Error('Gagal meminta pairing code setelah ' + attempt + ' percobaan: ' + (e?.message || e));
				logger?.warn?.(`requestPairingCode gagal (percobaan ${attempt}): ${e?.message || e} — retry dalam ${5 * attempt}s...`);
...
global.db = new Low(
	/https?:\/\//.test(opts['db'] || '') ?
		new (await import('./lib/cloudDBAdapter.js')).default(opts['db']) : /mongodb/.test(opts['db']) ?
			new mongoDB(opts['db']) : /json/i.test(opts['db'] || '') ?
				new JSONFile(`${dbBase}.json`) :
				new (await import('./lib/sqliteDB.js')).default(`${dbBase}.sqlite`, [`${dbBase}.json`, path.join(__dirname, 'database.json')])
)
    ...
    if (process.argv.includes('--qr')) {
        ...
        setTimeout(async () => {
            try {
                ...
                console.log(chalk.red('Gagal mendapatkan pairing code: ' + (e?.message || e)))
            }
        }, 5000)
    }
}
...
async function _quickTest() {
	...
	// import './lib/sticker.js'.support = s
