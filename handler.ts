...
import simple, { isNewsletterJid } from './lib/simple.ts'
...
export default {
    async handler(chatUpdate: { messages: WaMessage[] }) {
        ...
        try {
            ...
            if (m.isGroup && !global.db.data._migratedDetect) {
                ...
                for (const [jid, chat] of Object.entries(global.db.data.chats)) {
                    const c = chat
                    if (jid.endsWith('@g.us') && c && !c.detect) c.detect = true;
                }
            }
            ...
            if (m.messageStubType && m.isGroup) {
                ...
                if (chat && chat.detect) {
                    ...
                    if (m.messageStubType === 21) {
                        ...
                        // messageStubParameters[0] = picture action: 'set' | 'delete' | 'set_avatar'
                        ...
                        text = m.messageStubParameters[0] === 'on' 
                            ...
                            : '```Group info can now be modified by all participants```'
                    } else if (m.messageStubType === 26) {
                        text = m.messageStubParameters[0] === 'on' 
                            ? '```Group has been closed, only admins can send messages```' 
                            : '```Group has been opened, all participants can send messages```'
                    }
                    if (text) {
            ...
            try {
                ...
                if (typeof user !== 'object') global.db.data.users[m.sender] = {}
                 if (user) {
                   ...
                   if (!isNumber(user.laststringclaim))
                     user.laststringclaim = 0;
                   ...
                   if (!isNumber(user.weapondurability))
                     user.weapondurability = 0;
                   ...
                   if (!isNumber(user.kapakdurability))
                     user.kapakdurability = 0;
                   if (!isNumber(user.axe)) user.axe = 0;
                   if (!isNumber(user.axedurability)) user.axedurability = 0;
                   if (!isNumber(user.pisau)) user.pisau = 0;
                   if (!isNumber(user.pisaudurability))
                   ...
                   if (!user.registered) {
                     if (!("name" in user))
                       user.name = await this.getName(m.sender);
                 ...
                 if (chat) {
                   ...
                   if (!("antiStickernokick" in chat))
                     chat.antiStickernokick = false;
                   ...
                   if (!("antilinkignokick" in chat))
                     chat.antilinkignokick = false;
                   if (!("antilinkfb" in chat)) chat.antilinkfb = false;
                   if (!("antilinkfbnokick" in chat))
                     chat.antilinkfbnokick = false;
                   if (!("antilinktwit" in chat)) chat.antilinktwit = false;
                   if (!("antilinktwitnokick" in chat))
                     chat.antilinktwitnokick = false;
                   if (!("antilinkyt" in chat)) chat.antilinkyt = false;
                   if (!("antilinkytnokick" in chat))
                     chat.antilinkytnokick = false;
                   if (!("antilinktele" in chat)) chat.antilinktele = false;
                   if (!("antilinktelenokick" in chat))
                     chat.antilinktelenokick = false;
                   if (!("antilinkwame" in chat)) chat.antilinkwame = false;
                   if (!("antilinkwamenokick" in chat))
                     chat.antilinkwamenokick = false;
                   if (!("antilinkall" in chat)) chat.antilinkall = false;
                   if (!("antilinkallnokick" in chat))
                     chat.antilinkallnokick = false;
                   if (!("antilinktt" in chat)) chat.antilinktt = false;
                   if (!("antilinkttnokick" in chat))
                     chat.antilinkttnokick = false;
        ...
        if (m.id.startsWith('3EB0') || (m.id.startsWith('BAE5') && m.id.length === 16 || m.isZapo && m.fromMe)) return;	
		 ...
		 let isROwner = [global.conn?.user?.jid, ...(global.owner || [])]
            ...
            const chatMeta = conn.chats[m.chat] as { metadata?: unknown } | undefined;
            const groupMetadata = (m.isGroup ? chatMeta?.metadata || (await this.groupMetadata(m.chat).catch((_) => null)) : {}) || {};
            ...
            for (let name in global.plugins) {
                ...
                if ((usedPrefix = (match[0] || '')[0])) {
                    ...
                    if (plugin.register == true && _user.registered == false) { // Registered only
                        fail('unreg', m, this)
                        continue
                    }
                    ...
                    m.isCommand = true
                    let xp = 'exp' in plugin ? parseInt(String(plugin.exp)) : 17 // XP earned per command
                    if (xp > 200) m.reply('Ngecit -_-')
                    else m.exp += xp
                    if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < Number(plugin.limit)) {
                        this.reply(m.chat, `Limit anda habis, silahkan beli melalui *${usedPrefix}buy* atau beli di *${usedPrefix}shop*`, m)
                        continue // Limit used up
                    }
   ...
   async participantsUpdate({ id, participants, action }) {
    ...
    async delete({ remoteJid, fromMe, id, participant }) {
        ...
        let chats = Object.entries(this.chats).find(([user, data]) => {
            const d = data as { messages?: Record<string, unknown> };
            return d.messages && d.messages[id];
        })
        ...
        let msg = chats ? (chats[1] as { messages?: Record<string, unknown> }).messages?.[id] : null
...
fs.watchFile(file, () => {
    fs.unwatchFile(file)
    console.log(chalk.redBright("Update 'handler.ts'"))
